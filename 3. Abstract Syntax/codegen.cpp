...
llvm::Value *AST::BreakExpre::codegen(CodeGenContext &context) {
  context.builder.CreateBr(std::get<1>(context.loopStack.top()));
  return llvm::Constant::getNullValue(
      llvm::Type::getInt64Ty(context.context));  // return nothing
}

llvm::Value *AST::ForExpre::codegen(CodeGenContext &context) {
...
llvm::Value *AST::SeqExpre::codegen(CodeGenContext &context) {
  llvm::Value *last = nullptr;
  for (auto &Expre : Expres_) last = Expre->codegen(context);
  return last;
}

llvm::Value *AST::LetExpre::codegen(CodeGenContext &context) {
...
llvm::Value *AST::NilExpre::codegen(CodeGenContext &context) {
  return llvm::ConstantPointerNull::get(context.nilType);
}

llvm::Value *AST::VarExpre::codegen(CodeGenContext &context) {
  auto var = var_->codegen(context);
  if (!var) return nullptr;
  return context.builder.CreateLoad(var, var->getName());
}
llvm::Value *AST::AssignExpre::codegen(CodeGenContext &context) {
  auto var = var_->codegen(context);
  if (!var) return nullptr;
  auto Expre = Expre_->codegen(context);
  if (!Expre) return nullptr;
  context.checkStore(Expre, var);
  return Expre;  // var is a pointer, should not return
}

llvm::Value *AST::IfExpre::codegen(CodeGenContext &context) {
...
llvm::Value *AST::WhileExpre::codegen(CodeGenContext &context) {
  auto function = context.builder.GetInsertBlock()->getParent();
  auto testBB = llvm::BasicBlock::Create(context.context, "test", function);
  auto loopBB = llvm::BasicBlock::Create(context.context, "loop", function);
  auto nextBB = llvm::BasicBlock::Create(context.context, "next", function);
...
llvm::Value *AST::CallExpre::codegen(CodeGenContext &context) {
...
llvm::Value *AST::ArrayExpre::codegen(CodeGenContext &context) {
  auto function = context.builder.GetInsertBlock()->getParent();
  ...
  auto indExpretr = context.createEntryBlockAlloca(
      function, llvm::Type::getInt64Ty(context.context), indexName);
  // before loop:
  context.builder.CreateStore(zero, indExpretr);
  ...
  auto index = context.builder.CreateLoad(indExpretr, indexName);
  ...
  auto nextVar = context.builder.CreateAdd(
      ...
      "nextvar");
  context.builder.CreateStore(nextVar, indExpretr);
...
llvm::Value *AST::Var_subs::codegen(CodeGenContext &context) {
  auto var = var_->codegen(context);
  auto Expre = Expre_->codegen(context);
  if (!var) return nullptr;
  var = context.builder.CreateLoad(var, "arrayPtr");
  return context.builder.CreateGEP(type_, var, Expre, "ptr");
}
llvm::Value *AST::Var_field::codegen(CodeGenContext &context) {
  auto var = var_->codegen(context);
  if (!var) return nullptr;
...
llvm::Value *AST::FieldExpre::codegen(CodeGenContext &context) {
  return Expre_->codegen(context);
}

llvm::Value *AST::RecordExpre::codegen(CodeGenContext &context) {
  ...
  size_t idx = 0u;
  for (auto &ty_field : fieldExpres_) {
    auto Expre = ty_field->codegen(context);
    if (!Expre) return nullptr;
    if (!ty_field->type_) return nullptr;
    auto elementPtr = context.builder.CreateGEP(
        ty_field->type_, var,
        llvm::ConstantInt::get(llvm::Type::getInt64Ty(context.context),
                               llvm::APInt(64, idx)),
        "elementPtr");
    context.checkStore(Expre, elementPtr);
    // context.builder.CreateStore(Expre, elementPtr);
    ++idx;
  }
  return var;
}

llvm::Value *AST::StringExpre::codegen(CodeGenContext &context) {
...
// TODO: Static link
llvm::Value *AST::Fun_dec::codegen(CodeGenContext &context) {
...
llvm::Value *AST::Var_dec::codegen(CodeGenContext &context) {
...
llvm::Value *AST::Var_dec::read(CodeGenContext &context) const {
  size_t currentLevel = context.currentLevel;
  auto staticLink = context.staticLink.begin();
  llvm::Value *value = context.currentFrame;
...
llvm::Value *AST::Ty_dec::codegen(CodeGenContext &context) {
  return llvm::Constant::getNullValue(llvm::Type::getInt64Ty(context.context));
}

llvm::Value *AST::BinaryExpre::codegen(CodeGenContext &context) {
